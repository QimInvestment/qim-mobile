...
const CustomBottomTabBar = ({ state, descriptors, navigation }:BottomTabBarProps)=> {
  ...
  return (
    <View style={{ flexDirection: 'row', backgroundColor:theme.BottomNavBackgroundColor, height:90, paddingTop:10, paddingBottom:20 }}>
      {state.routes.map((route, index) => {
        ...
        const icon =  options.tabBarIcon ? options.tabBarIcon({focused:isFocused, color:isFocused ? '#fff' : theme.BottomNavLabelColor as string, size:22}) : <MaterialIcons name="lock-outline" size={22} color={isFocused ? '#fff' : theme.BottomNavLabelColor} />
